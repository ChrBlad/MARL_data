...
number=108*7#3240#108#3240#12400#
number_n=108*1#108*10
n=0
m=number
n_ref=0
m_ref=number
...
with open('output_zone1_SARL.csv', 'r') as csvFile:
    reader = csv.reader(csvFile)
    for row in reader:
...
with open('output_zone2_SARL.csv', 'r') as csvFile:
    reader = csv.reader(csvFile)
    for row in reader:
...
with open('output_zone3_SARL.csv', 'r') as csvFile:
    reader = csv.reader(csvFile)
    for row in reader:
...
with open('output_zone4_SARL.csv', 'r') as csvFile:
    reader = csv.reader(csvFile)
    for row in reader:
...
with open('output_actions_SARL.csv', 'r') as csvFile:
    reader = csv.reader(csvFile)
    for row in reader:
...
df_actions_com.columns = ["Valve1", "Supplytemp","Reward","Hardcon","price_reward","sum_mix_reward","change_in_Supply_temp"]
df_actions_ref = pd.DataFrame(data_actions_ref)
df_actions_ref.columns = ["Valve1", "Supplytemp","Reward","Hardcon","price_reward","sum_mix_reward","change_in_Supply_temp"]
...
SS_ref_1=len(data_zone1_ref)
SS_action=len(data_actions)
print(SS_action)
SS_action_ref=len(data_zone1_com)
print(SS_action_ref)
SS=min(SS,SS_ref,SS_ref_1)
...
df_zone1_com.columns = ["Troom", "Tamb", "Valve","Power1","Reward","action",'Price','Power','Powerwater','Hcv',"time"]

df_zone2_com = pd.DataFrame(data_zone2_com)
df_zone2_com.columns = ["Troom", "Tamb", "Valve","Power1","Reward","action",'Price','Power','Powerwater','Hcv',"time"]


df_zone3_com = pd.DataFrame(data_zone3_com)
df_zone3_com.columns = ["Troom", "Tamb", "Valve","Power1","Reward","action",'Price','Power','Powerwater','Hcv',"time"]

df_zone4_com = pd.DataFrame(data_zone4_com)
df_zone4_com.columns = ["Troom", "Tamb", "Valve","Power1","Reward","action",'Price','Power','Powerwater','Hcv',"time"]





df_zone1_ref = pd.DataFrame(data_zone1_ref)
df_zone1_ref.columns = ["Troom", "Tamb", "Valve","Power1","Reward","action",'Price','Power','Powerwater','Hcv',"time"]

df_zone2_ref = pd.DataFrame(data_zone2_ref)
df_zone2_ref.columns = ["Troom", "Tamb", "Valve","Power1","Reward","action",'Price','Power','Powerwater','Hcv',"time"]


df_zone3_ref = pd.DataFrame(data_zone3_ref)
df_zone3_ref.columns = ["Troom", "Tamb", "Valve","Power1","Reward","action",'Price','Power','Powerwater','Hcv',"time"]
df_zone4_ref = pd.DataFrame(data_zone4_ref)
df_zone4_ref.columns = ["Troom", "Tamb", "Valve","Power1","Reward","action",'Price','Power','Powerwater','Hcv',"time"]
...
df_zone1_ref=df_zone1_ref.astype(float)
df_zone2_ref=df_zone2_ref.astype(float)
df_zone3_ref=df_zone3_ref.astype(float)
df_zone4_ref=df_zone4_ref.astype(float)



df_actions = df_actions.astype(float)
df_actions_com = df_actions_com.astype(float)
df_actions_ref = df_actions_ref.astype(float)
...
df_zone1['Price']=df_zone1['Price']+df_zone2['Price']+df_zone3['Price']+df_zone4['Price']

df_zone1_com['Price']=df_zone1_com['Price']+df_zone2_com['Price']+df_zone3_com['Price']+df_zone4_com['Price']

df_zone1_ref['Price']=df_zone1_ref['Price']+df_zone2_ref['Price']+df_zone3_ref['Price']+df_zone4_ref['Price']
...
df_actions['Reward']=df_actions['Reward']+df_actions['Hardcon']+df_zone1['Reward']+df_zone2['Reward']+df_zone3['Reward']+df_zone4['Reward']+df_zone1['Hcv']+df_zone2['Hcv']+df_zone3['Hcv']+df_zone4['Hcv']#+df_actions['price_reward']#+df_actions['Hardcon']#+df_zone2['Reward']
df_actions_com['Reward']=df_actions_com['Reward']+df_actions_com['Hardcon']+df_zone1_com['Hcv']+df_zone2_com['Hcv']+df_zone3_com['Hcv']+df_zone4_com['Hcv']#+df_actions_com['price_reward']#-df_zone2_com['Reward']-df_zone1_com['Reward']-df_zone3_com['Reward']-df_zone4_com['Reward']#-df_actions_com['price_reward']#+df_zone2_com['Reward']
#df_actions['Reward']=df_actions['Reward']-df_actions_com['Reward']
df_actions_ref['Reward']=df_actions_ref['Reward']#+df_zone1_ref['Reward']+df_zone2_ref['Reward']+df_zone3_ref['Reward']+df_zone4_ref['Reward']
...
df_zone1_ref['Tamb']=df_zone1_ref['Tamb']-273.15
df_zone2_ref['Troom']=df_zone2_ref['Troom']-273.15
df_zone2_ref['Tamb']=df_zone2_ref['Tamb']-273.15
df_zone3_ref['Troom']=df_zone3_ref['Troom']-273.15
df_zone3_ref['Tamb']=df_zone3_ref['Tamb']-273.15
df_zone4_ref['Troom']=df_zone4_ref['Troom']-273.15
df_zone4_ref['Tamb']=df_zone4_ref['Tamb']-273.15
...
def Evaluation_of_reward(m,n,SS,n_ref,m_ref,n_ref_1,m_ref_1,df_zone1,df_zone1_com,df_zone1_ref,reward20,reward20_ref,reward20_ref_1,number,fig_num):
    ...
    reward_ref = df_zone1_ref['Reward']
    
    while m<SS: 
        reward20.append(sum(reward[n:m]/number))
        n=n+number
        m=m+number

    while m_ref<SS: 
        reward20_ref.append(sum(reward_com[n_ref:m_ref]/number))
        n_ref=n_ref+number
        m_ref=m_ref+number

    while m_ref_1<SS: 
        reward20_ref_1.append(sum(reward_ref[n_ref_1:m_ref_1]/number))
        n_ref_1=n_ref_1+number
        m_ref_1=m_ref_1+number

    f6= plt.figure(fig_num)
    X = list(range(len(reward20)))#*number
    X_ref = list(range(len(reward20_ref)))#*number
    X_ref_1 = list(range(len(reward20_ref_1)))#*number
    X = [i * (number/108) for i in X]
    X_ref = [i * (number/108) for i in X_ref]
    X_ref_1 = [i * (number/108) for i in X_ref_1]
    plt.plot(X,reward20,label='MARL', alpha=0.7)
    plt.plot(X_ref,reward20_ref,label='SARL', alpha=0.7)
    plt.plot(X_ref_1,reward20_ref_1,label='Traditional Control', alpha=0.7)
    plt.legend()
...
def Evaluation_of_price(m_pr,n_pr,SS,n_pr_ref,m_pr_ref,df_zone1,df_zone1_com,price20,price20_ref,number,fig_num):
    ...
    while m_pr<SS: 
        price20.append((sum(price[n_pr:m_pr])/number)*10*7)
        n_pr=n_pr+number
        m_pr=m_pr+number
    
    price_ref = df_zone1_com['Price']
    while m_pr_ref<SS: 
        price20_ref.append(sum((price_ref[n_pr_ref:m_pr_ref])/number)*10*7)
    ...
    plt.plot(X,price20,label='MARL', alpha=0.7)
    plt.plot(X_ref,price20_ref,label='Traditional control policy', alpha=0.7)
    plt.plot(X_sav,savings_price,label='Savings', alpha=0.7)
    plt.ylabel('price[Euro pr week]')
...
Evaluation_of_price(m_pr,n_pr,SS,n_pr_ref,m_pr_ref,df_zone1,df_zone1_ref,price20,price20_ref,number,30)
Evaluation_of_price(m_pr,n_pr,SS,n_pr_ref,m_pr_ref,df_zone2,df_zone2_ref,price20,price20_ref,number,31)
Evaluation_of_price(m_pr,n_pr,SS,n_pr_ref,m_pr_ref,df_zone3,df_zone3_ref,price20,price20_ref,number,32)
Evaluation_of_price(m_pr,n_pr,SS,n_pr_ref,m_pr_ref,df_zone4,df_zone4_ref,price20,price20_ref,number,33)


#Evaluation_of_price(m_pr,n_pr,SS,n_pr_ref,m_pr_ref,df_zone1_com,df_zone1_ref,price20,price20_ref,number,30)
Evaluation_of_price(m_pr,n_pr,SS,n_pr_ref,m_pr_ref,df_zone2_com,df_zone2_ref,price20,price20_ref,number,31)
Evaluation_of_price(m_pr,n_pr,SS,n_pr_ref,m_pr_ref,df_zone3_com,df_zone3_ref,price20,price20_ref,number,32)
Evaluation_of_price(m_pr,n_pr,SS,n_pr_ref,m_pr_ref,df_zone4_com,df_zone4_ref,price20,price20_ref,number,33)
