...
#         self.env = FMU_stepping
    ...
    def step(self, action):
        ...
        itnum1,itnum2,itnum3,itnum4,itmun_mix  = self.Env_master_script.zone1(action)
        ...
        while stay:
            ...
            if itnum1 == itnum2 == itnum3 ==itnum4 == itmun_mix:
                ...
                stay = True
        out,actions_to_env,ref = self.Env_master_script.sendaction(action)
        hard_v1=self.Env_master_script.Hardconstraint_send_V1()
        last_reward,Price1,Power1 = self.reward_calculator.calculate_reward_Zone1(out,actions_to_env,hard_v1,timeofday,itnum1)
        state = self.ai_input_provider.calculate_ai_input_Zone1(out,actions_to_env,timeofday,Price1,hard_v1)       
        default_observation = state
        obs, reward  = default_observation, last_reward 
        ...
        data = out['RoomTemperature1'].tolist(),out['Tamb1'].tolist(),out['Valveout1'].tolist(),out['Power1'].tolist(), last_reward,np.float64(action),np.float64(Price1),Power1,out['Powerwater1'].tolist(),hard_v1,simulation_time
