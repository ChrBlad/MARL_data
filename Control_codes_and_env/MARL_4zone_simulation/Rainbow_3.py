...
class DQNAgent1:
    ...
    def select_action(self, state: np.ndarray) -> np.ndarray:
        ...
        self.selected_action=self.Env_master_script.Hardconstraint_for_V3(selected_action)
        aa=selected_action
        if not self.is_test:
    ...
    def savefun_Loss(self,data):
        ...
        self.dataloss_3.append(data)
        if self.nn == 200:
            with open("Loss_3.csv",'a', newline='') as resultFile:
                wr = csv.writer(resultFile)
                wr.writerows(self.dataloss_3)
            resultFile.close()
            self.dataloss_3 =[]
            self.nn =0
