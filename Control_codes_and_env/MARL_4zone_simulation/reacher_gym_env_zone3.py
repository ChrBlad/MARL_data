...
#         self.env = FMU_stepping
    ...
    def step(self, action):
        ...
        #print(action)
        itnum3  = self.Env_master_script.zone3(action)
        out,actions_to_env,tref = self.Env_master_script.outzone3(itnum3)
        outw=out.tolist()
        while outw[0] == 0: 
            out,actions_to_env,tref = self.Env_master_script.outzone3(itnum3)
            outw=out.tolist()
        ...
        last_reward3,Price3,Power3 = self.reward_calculator.calculate_reward_Zone3(out,actions_to_env,hard_v3,timeofday,itnum3)
        state = self.ai_input_provider.calculate_ai_input_Zone3(out,actions_to_env,timeofday,Price3,hard_v3)       
        default_observation = state
        obs, reward  = default_observation, last_reward3
