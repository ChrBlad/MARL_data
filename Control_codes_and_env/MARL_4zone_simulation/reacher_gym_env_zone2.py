...
#         self.env = FMU_stepping
    ...
    def step(self, action):
        ...
        #print(action)
        itnum2  = self.Env_master_script.zone2(action)
        out,actions_to_env,tref = self.Env_master_script.outzone2(itnum2)
        outw=out.tolist()
        while outw[0] == 0: 
            out,actions_to_env,tref = self.Env_master_script.outzone2(itnum2)
            outw=out.tolist()
        ...
        last_reward2,Price2,Power2 = self.reward_calculator.calculate_reward_Zone2(out,actions_to_env,hard_v2,timeofday,itnum2)
        state = self.ai_input_provider.calculate_ai_input_Zone2(out,actions_to_env,timeofday,Price2,hard_v2)       
        default_observation = state
        obs, reward  = default_observation, last_reward2
