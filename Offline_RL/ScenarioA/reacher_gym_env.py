...
#         self.env = FMU_stepping
    ...
    def step(self, action):
        if self.NN_world_bool == 0:
            ...
            itnum1,itnum2,itnum3,itnum4,itmun_mix  = self.Env_master_script.zone1(action,self.NN_world_bool,type=1)
            #Masked_action = self.Env_master_script.Hardconstraint_for_V1(action)
            if itnum1 == itnum2 == itnum3 ==itnum4 == itmun_mix:
                stay = False
            else:
                stay = True
              #  print("1")
            while stay:
                itnum1,itnum2,itnum3,itnum4,itmun_mix = self.Env_master_script.check()
                if itnum1 == itnum2 == itnum3 ==itnum4 == itmun_mix:
                    stay = False
                else:
                    stay = True
            ...
            reward,Price1,Power1,hard_conV1 = self.reward_calculator.calculate_reward_Zone1(out,action,actions_to_env,done,self.NN_world_bool,hard_v1,timeofday,self.NN_world_bool)  
            obs = self.ai_input_provider.calculate_ai_input_Zone1(out,actions_to_env,timeofday,Price1)       
            info = {"text_observation": 'allgood', "brain_info": 0}
            data = out['RoomTemperature1'].tolist(),out['Tamb1'].tolist(),out['Valveout1'].tolist(),out['Power1'].tolist(), reward,np.float64(action),np.float64(Price1),Power1,out['Powerwater1'].tolist(),hard_conV1,timeofday,simulation_time
            self.ai_input_provider.savefun_Zone1(data)
            ...
            while stay:
                itnum1,itnum2,itnum3,itnum4,itmun_mix = self.Env_master_script.check()
                #print('stuck in Z1_NN',itnum1,itnum2,itnum3,itnum4,itmun_mix)
                if itnum1 == itnum2 == itnum3 ==itnum4 == itmun_mix:
                    stay = False
                else:
                    stay = True
            ...
            hard_v1=self.Env_master_script.Hardconstraint_send_V1()
            reward,Price1,Power1,hard_conV1 = self.reward_calculator.calculate_reward_Zone1(out,action,actions_to_env,done,self.NN_world_bool,hard_v1,timeofday_NN,self.NN_world_bool)  
            obs = self.ai_input_provider.calculate_ai_input_Zone1(out,actions_to_env,timeofday_NN,Price1)
            info = {"text_observation": 'allgood', "brain_info": 0}
            data = float(out['RoomTemperature1']),float(out['Tamb1']),float(out['Valveout1']),float(out['Power1']), float(reward),np.float64(action),np.float64(Price1),Power1,float(out['Powerwater1']),hard_conV1,timeofday_NN,NN_world_simulation_time,done
            self.ai_input_provider.savefun_Zone1_NN(data)
