...
#         self.env = FMU_stepping
    ...
    def step(self, action):
        if self.NN_world_bool == 0:
            ...
            itnum1,itnum2,itnum3,itnum4,itmun_mix  = self.Env_master_script.zone4(action,self.NN_world_bool,type=1)
            out,actions_to_env,tref,self.NN_world_bool,timeofday_NN,done,move = self.Env_master_script.outzone4(itnum4)
            ...
            hard_v=self.Env_master_script.Hardconstraint_send_V4()
            reward,Price,Power,hard_conV = self.reward_calculator.calculate_reward_Zone4(out,action,actions_to_env,done,self.NN_world_bool,hard_v,timeofday,self.NN_world_bool)  
            obs = self.ai_input_provider.calculate_ai_input_Zone4(out,actions_to_env,timeofday,Price)       
            info = {"text_observation": 'allgood', "brain_info": 0}
            data = out['RoomTemperature4'].tolist(),out['Tamb1'].tolist(),out['Valveout4'].tolist(),out['Power4'].tolist(), reward,np.float64(action),np.float64(Price),Power,out['Powerwater4'].tolist(),hard_conV,timeofday,simulation_time
            self.ai_input_provider.savefun_Zone4(data)
            ...
            hard_v=self.Env_master_script.Hardconstraint_send_V4()
            reward,Price,Power,hard_conV = self.reward_calculator.calculate_reward_Zone4(out,action,actions_to_env,done,self.NN_world_bool,hard_v,timeofday_NN,self.NN_world_bool)  
            obs = self.ai_input_provider.calculate_ai_input_Zone4(out,actions_to_env,timeofday_NN,Price)
            info = {"text_observation": 'allgood', "brain_info": 0}
            ...
            self.ai_input_provider.savefun_Zone4_NN(data)
    ...
    def reset(self):
        if self.NN_world_bool == 0:
            ...
            Masked_action = self.Env_master_script.Hardconstraint_for_V4(action)
            itnum1,itnum2,itnum3,itnum4,itmun_mix  = self.Env_master_script.zone4(action,self.NN_world_bool,type=1)
            out,actions_to_env,tref,self.NN_world_bool,timeofday_NN,done,move = self.Env_master_script.outzone4(itnum4)
            outw=out.tolist()
            while outw[0] == 0: 
