...
class CustomReacherEnv_zone2(gym.Env):
    '''Custom wrapper
    '''
    metadata = {'render.modes': ['human']}

    def __init__(self,Env_master_script, *args, **kwargs):
        super(CustomReacherEnv_zone2, self).__init__()
...
#         self.env = FMU_stepping
    ...
    def step(self, action):
        if self.NN_world_bool == 0:
            ...
            itnum1,itnum2,itnum3,itnum4,itmun_mix  = self.Env_master_script.zone2(action,self.NN_world_bool,type=1)
            out,actions_to_env,tref,self.NN_world_bool,timeofday_NN,done,move = self.Env_master_script.outzone2(itnum2)
            ...
            hard_v=self.Env_master_script.Hardconstraint_send_V2()
            reward,Price,Power,hard_conV = self.reward_calculator.calculate_reward_Zone2(out,action,actions_to_env,done,self.NN_world_bool,hard_v,timeofday,self.NN_world_bool)  
            obs = self.ai_input_provider.calculate_ai_input_Zone2(out,actions_to_env,timeofday,Price)       
            info = {"text_observation": 'allgood', "brain_info": 0}
            data = out['RoomTemperature2'].tolist(),out['Tamb1'].tolist(),out['Valveout2'].tolist(),out['Power2'].tolist(), reward,np.float64(action),np.float64(Price),Power,out['Powerwater2'].tolist(),hard_conV,timeofday,simulation_time
            self.ai_input_provider.savefun_Zone2(data)
            ...
            hard_v=self.Env_master_script.Hardconstraint_send_V2()
            reward,Price,Power,hard_conV = self.reward_calculator.calculate_reward_Zone2(out,action,actions_to_env,done,self.NN_world_bool,hard_v,timeofday_NN,self.NN_world_bool)  
            obs = self.ai_input_provider.calculate_ai_input_Zone2(out,actions_to_env,timeofday_NN,Price)
            info = {"text_observation": 'allgood', "brain_info": 0}
            data = float(out['RoomTemperature2']),float(out['Tamb1']),float(out['Valveout2']),float(out['Power2']), float(reward),np.float64(action),np.float64(Price),Power,float(out['Powerwater2']),hard_conV,timeofday_NN,NN_world_simulation_time,done
            self.ai_input_provider.savefun_Zone2_NN(data)
    ...
    def reset(self):
        if self.NN_world_bool == 0:
            ...
            Masked_action = self.Env_master_script.Hardconstraint_for_V2(action)
            itnum1,itnum2,itnum3,itnum4,itmun_mix  = self.Env_master_script.zone2(action,self.NN_world_bool,type=1)
            out,actions_to_env,tref,self.NN_world_bool,timeofday_NN,done,move = self.Env_master_script.outzone2(itnum2)
            outw=out.tolist()
            while outw[0] == 0: 
